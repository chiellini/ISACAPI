...
type UpdateAffiliateAgentStatusRequest struct {
	Status string `json:"status" binding:"required"`
}
type RejectAffiliateWithdrawalRequest struct {
	Reason string `json:"reason" binding:"required"`
}
type MarkAffiliateWithdrawalPaidRequest struct {
	ActualCurrency    string  `json:"actual_currency" binding:"required"`
