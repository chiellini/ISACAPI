...
func (r *userRepository) PrepareResearchGroupUserDeletion(ctx context.Context, userID, actorUserID int64) error {
	// Research-group persistence uses PostgreSQL advisory locks and JSONB SQL.
	// SQLite is only used by Ent repository tests and cannot execute this path.
...
func (r *userRepository) GetFirstAdmin(ctx context.Context) (*service.User, error) {
	m, err := r.client.User.Query().
		Where(
			dbuser.RoleIn(service.RoleAdmin, service.RoleAdminProvider),
