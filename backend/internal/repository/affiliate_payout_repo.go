...
func insertAffiliateAdminAudit(ctx context.Context,c affiliateQueryExecer,operatorID int64,targetID,withdrawalID *int64,action,key,detail string)error{_,err:=c.ExecContext(ctx,`INSERT INTO user_affiliate_admin_audits(operator_user_id,target_user_id,withdrawal_id,action,idempotency_key,detail,created_at) VALUES($1,$2,$3,$4,$5,$6::jsonb,NOW())`,operatorID,nullableInt64(targetID),nullableInt64(withdrawalID),action,nullableTrimmedString(key),detail);return err}
...
func nullableTrimmedString(v string)any{if strings.TrimSpace(v)==""{return nil};return strings.TrimSpace(v)}
