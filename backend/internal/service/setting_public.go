...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	...
	return &PublicSettings{
		...
		BalanceRechargeMultiplier:           normalizeBalanceRechargeMultiplier(pcParseFloat(settings[SettingBalanceRechargeMult], defaultBalanceRechargeMultiplier)),
...
// GetModelPlazaRuntime reads the model-plaza switches directly from the settings
// store. It fails closed because the feature is opt-in.
...
// PublicStatusRuntime is the lightweight view of the public status feature
// switch consumed by the no-auth status handler.
type PublicStatusRuntime struct {
	Enabled bool
}

// GetPublicStatusRuntime reads the public status feature switch directly from
// the settings store. Fail-closed: on error returns Enabled=false, matching
// the opt-in default.
