...
// maxReasoningEffortBillingMultiplier keeps legacy callers compatible while
// pricing now stores the complete reasoning-effort multiplier map.
func maxReasoningEffortBillingMultiplier(model, effort string, pricing *ModelPricing) float64 {
	if pricing == nil {
		return 1
	}
	return reasoningEffortBillingMultiplier(effort, pricing.ReasoningEffortMultipliers)
}
