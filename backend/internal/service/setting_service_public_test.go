...
func TestSettingService_GetPublicSettings_ExposesBalanceRechargeMultiplier(t *testing.T) {
	repo := &settingPublicRepoStub{
		values: map[string]string{
			SettingBalanceRechargeMult: "5.5",
		},
	}
	svc := NewSettingService(repo, &config.Config{})
	...
	settings, err := svc.GetPublicSettings(context.Background())
	require.NoError(t, err)
	require.Equal(t, 5.5, settings.BalanceRechargeMultiplier)
}

func TestSettingService_GetPublicSettings_DefaultsInvalidBalanceRechargeMultiplier(t *testing.T) {
	repo := &settingPublicRepoStub{
		values: map[string]string{
	...
	settings, err := svc.GetPublicSettings(context.Background())
	require.NoError(t, err)
	require.Equal(t, defaultBalanceRechargeMultiplier, settings.BalanceRechargeMultiplier)
}
