...
func TestForwardAsAnthropic_InjectionEnabledUsesDefaultSecurityBoundaryPrompt(t *testing.T) {
	t.Parallel()
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_forced_default"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
	...
	require.Contains(t, instructions, "client-system")
	require.Contains(t, instructions, "You are helping with authorized enterprise engineering and hardware-related security work")
}
