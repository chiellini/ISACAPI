...
- **官网**: [isacai.space](https://isacai.space)
- **邮箱**: chiellini.lee@gmail.com

## 项目概述

ISACAPI 是一个 AI API 网关平台，用于分发和管理 AI 产品订阅的 API 配额。用户通过平台生成的 API Key 调用上游 AI 服务，平台负责鉴权、计费、负载均衡和请求转发。
...
使用 Docker Compose 在本地构建并启动服务：

```bash
sudo docker compose -f docker-compose.local.yml -f docker-compose.build.yml up -d --build --force-recreate sub2api
```

## 生态项目

围绕 ISACAPI 的社区扩展与集成项目：

| 项目 | 说明 | 功能 |
|------|------|------|
| ~~[Sub2ApiPay](https://github.com/touwaeriol/sub2apipay)~~ | ~~自助支付系统~~ | **已内置** — 支付功能已集成到 ISACAPI 中，无需独立部署。详见 [支付配置指南](docs/PAYMENT_CN.md) |
...
通过 Nginx 反向代理 ISACAPI（或 CRS 服务）并搭配 Codex CLI 使用时，需要在 Nginx 配置的 `http` 块中添加：
...
ISACAPI 支持 [Antigravity](https://antigravity.so/) 账户，授权后可通过专用端点访问 Claude 和 Gemini 模型。
...
## 虚拟模型别名（Model Aliases）

网关可对外暴露"虚拟模型名"，命中后把请求改写为真实上游模型 + 预设推理档位 / 服务档位 / 计费倍率。内置两个别名：

| 别名 | 上游模型 | 推理档位 | 服务档位 | 计费 |
|------|----------|----------|----------|------|
| `isac-gpt-fast` | `gpt-5.5` | extra high (`xhigh`) | `priority`（fast 提速） | 1.5× |
| `isac-gpt-best` | `gpt-5.5` | extra high (`xhigh`) | — | 1× |

客户端通过 `/v1/responses` 或 `/v1/chat/completions` 调用（OpenAI 分组的 `GET /v1/models` 也会列出）。需要该分组下有账号支持目标上游模型。

### 如何修改映射

编辑运行时配置 `data/config.yaml`（挂载到 `/app/data/config.yaml`）里的 `model_aliases`，然后重启容器即可——**无需重新 build 镜像**：

```yaml
model_aliases:
  - alias: "isac-gpt-fast"
    target_model: "gpt-5.5"
    reasoning_effort: "xhigh"   # low / medium / high / xhigh(extra high)；留空=不注入
    service_tier: "priority"    # priority(=fast) / flex / auto；留空=不注入
    rate_multiplier: 1.5        # <=0 或省略=不改计费；>0 时为唯一加价
    platform: "openai"          # 留空=openai
  - alias: "isac-gpt-best"
    target_model: "gpt-5.5"
...
- 不配置 `model_aliases` → 使用内置默认（上面两条）；配置后完全覆盖默认表，可自由增删改别名。
- `rate_multiplier` 与定价配置解耦：`> 0` 时它是**唯一**加价。注入的 `service_tier=priority` 仍发给上游提速，但计费侧已中和，所以 `isac-gpt-fast` 正好是 `rate_multiplier × 基础价`。
- 首次启用本特性需重新 build 镜像（`--build`）；之后只改映射只需 `restart`。
- 完整字段说明见 [`deploy/config.example.yaml`](deploy/config.example.yaml)。
...
<a href="https://star-history.com/#chiellini/ISACAPI&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=chiellini/ISACAPI&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=chiellini/ISACAPI&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=chiellini/ISACAPI&type=Date" />
 </picture>
</a>
