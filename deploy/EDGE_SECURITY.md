...
The application-level compatibility default for
`security.trust_forwarded_ip_for_api_key_acl` is enabled for upgrades, but all
production Compose templates explicitly set it to `false`. While enabled, raw forwarding headers take over client-IP
