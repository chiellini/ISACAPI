...
Options:
  ...
  --config <path>            Squid config path. Default: /etc/squid/squid.conf
  --no-install               Do not install squid package
...
install_squid_rpm() {
  ...
  if rpm_install_package "$pm" squid; then
    return 0
  fi

  die "squid is not available from current ${pm} repositories. Install squid manually, then rerun with --no-install. If docker-ce-stable is broken, the script already tries to ignore it temporarily."
}
...
install_squid() {
  ...
  if command -v apt-get >/dev/null 2>&1; then
    ...
    install_squid_rpm dnf
  elif command -v yum >/dev/null 2>&1; then
    install_squid_rpm yum
  elif command -v zypper >/dev/null 2>&1; then
    zypper --non-interactive install squid
  elif command -v pacman >/dev/null 2>&1; then
    pacman -Sy --noconfirm squid
  else
    die "could not find a supported package manager. Install squid manually, then rerun with --no-install"
  fi
}
...
squid_http_port() {
  if [[ "$LISTEN_HOST" == "0.0.0.0" || "$LISTEN_HOST" == "::" ]]; then
    ...
    printf '[%s]:%s' "$LISTEN_HOST" "$PROXY_PORT"
  else
    printf '%s:%s' "$LISTEN_HOST" "$PROXY_PORT"
  fi
}
...
write_squid_config() {
  if [[ "$DRY_RUN" -eq 1 ]]; then
    info "Would write squid config: ${CONFIG_PATH}"
...

  {
    ...
    printf '# Machine 2 Squid proxy server for machine 1 AI and developer upstream traffic.\n\n'
    printf 'acl machine1_clients src'
    local client
    for client in "${ALLOW_CLIENTS[@]}"; do
      printf ' %s' "$client"
    done
    ...
    printf 'acl SSL_ports port 563\n'
    printf 'acl SSL_ports port 8443\n'
    printf 'acl Safe_ports port 80\n'
    printf 'acl Safe_ports port 443\n'
    printf 'acl Safe_ports port 563\n'
    printf 'acl Safe_ports port 8443\n'
    printf 'acl CONNECT method CONNECT\n'
    printf '\n'
    printf 'http_access deny !Safe_ports\n'
    printf 'http_access deny CONNECT !SSL_ports\n'
    printf 'http_access allow machine1_clients\n'
    printf 'http_access deny all\n'
    printf '\n'
    printf 'http_port %s\n' "$(squid_http_port)"
    printf 'cache deny all\n'
    printf 'via off\n'
    printf 'forwarded_for delete\n'
    printf 'visible_hostname isacapi-machine2\n'
    printf '\n'
  } >"$CONFIG_PATH"

  info "Wrote squid config: ${CONFIG_PATH}"
}

validate_squid_config() {
  if [[ "$DRY_RUN" -eq 1 ]]; then
    info "Would validate squid config"
    return 0
  fi
  ...
  if command -v squid >/dev/null 2>&1; then
    ...
    warn "squid command not found; skipped config validation"
  fi
}
...
restart_service() {
  ...
  if command -v systemctl >/dev/null 2>&1; then
    ...
    warn "Could not find systemctl or service. Start squid manually."
  fi
}
...
check_proxy() {
  ...
  if curl -sS --max-time 15 --proxy "http://127.0.0.1:${PROXY_PORT}" https://api.anthropic.com/ >/dev/null; then
    ...
    warn "Local proxy test failed. Check squid logs and machine 2 outbound network."
  fi
}
