...
describe('CC-Switch and one-click setup locale copy', () => {
  ...
  it.each(Object.entries(locales))('defines new setup copy for %s', (_locale, messages) => {
    ...
    expect(messages.keys.ccsFallback.macosHint).toBeTruthy()
    expect(messages.keys.ccsFallback.retryOpen).toBeTruthy()
    expect(messages.keys.ccsFallback.retryOpenDesc).toBeTruthy()
  ...
  it('does not let Japanese or Arabic fall back to English for the new visible strings', () => {
    for (const messages of [ja, ar]) {
      ...
      expect(messages.keys.ccsFallback.macosHint).not.toBe(en.keys.ccsFallback.macosHint)
      expect(messages.keys.ccsFallback.retryOpen).not.toBe(en.keys.ccsFallback.retryOpen)
