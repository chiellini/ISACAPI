export default {
    ...
    riskControl: {
      ...
      auditAPIFormatChatDesc: 'Calls /chat/completions for gray-zone review; timeouts, missing keys, network failures, or invalid results are blocked.',
      ...
      localSecurityScoringHint: 'Exact or combination matches from independent local rules accumulate risk scores. Scores at the block threshold reject immediately; observed scores are logged and continue to downstream moderation for contextual review.',
      ...
      localSecurityBlockScore: 'Block score',
      localSecurityBlockScoreHint: 'Reject the request at this score before billing, routing, or an upstream call.',
      preBlockFailureMode: 'When audit is unavailable',
      preBlockFailureModeHint: 'Only affects missing audit Keys, timeouts, or audit API errors. Local hard rules always run first.',
      ...
      localSecurityRulesHint: 'JSON array. Built-in local rules block only software cracking and reverse-engineering intents; action + target must appear close together. A rule matches when an exact term appears, all terms in "all" appear, or any action appears near any target. Optional score (1-100) overrides the rule default; distant combinations are downgraded to observation. Only models within the configured "Model scope" are checked. Rules run before billing, routing, and upstream calls.',
