export default {
    ...
    riskControl: {
      ...
      configHint: 'Use the configured Moderations or Chat Completions audit API to score request content and handle threshold hits by mode.',
      ...
      enabledHint: 'When off, the audit API and async queue are skipped; while global Risk Control remains on, highly specific local prompt-injection fingerprints still block immediately.',
      mode: 'Global Mode',
      ...
      modeOff: 'Off',
      modeOffDesc: 'The audit API and async review are disabled; while Risk Control is on, highly specific local prompt-injection fingerprints are still blocked and recorded locally.',
      auditAPIFormat: 'Audit API format',
      auditAPIFormatOpenAI: 'OpenAI Moderations',
      auditAPIFormatChat: 'Chat Completions (DeepSeek)',
      auditAPIFormatOpenAIDesc: 'Calls /v1/moderations for OpenAI or compatible moderation services.',
      auditAPIFormatChatDesc: 'Calls /chat/completions for contextual review. Local combination review failures never block by themselves; normal pre-block failures follow the setting below.',
      baseUrl: 'API Base URL',
      model: 'Model',
      apiKey: 'Audit API Key',
      apiKeys: 'Audit API Keys',
      ...
      hitRetentionDays: 'Hit and Error Retention (days)',
      ...
      auditErrorReason: 'Audit Error Reason',
      matchedKeyword: 'Matched Keyword / Rule',
      ...
      cleanupStats: 'Last cleanup deleted {hit} hits/errors and {nonHit} non-hits',
      ...
      tabs: {
        ...
        response: 'Hit Notice',
        riskThresholds: 'Risk Thresholds',
        keywords: 'Keyword Combination Review',
        retention: 'Retention',
      },
      blockedKeywords: 'Review keywords',
      blockedKeywordsPlaceholder: 'One keyword per line, e.g.:\nbadword1\nbadword2',
      blockedKeywordsDescription: 'Matching is case-insensitive. At least two distinct nearby keywords are required to trigger secondary API review; one keyword never blocks directly.',
      blockedKeywordsPreBlockHint: 'Keyword combination review only takes effect in "Pre-block" mode.',
      blockedKeywordsModeWarning: 'Current mode is "{mode}". Keyword combination review will not run until you switch to "Pre-block" mode.',
      ...
      localSecurityScoring: 'Local security risk scoring',
      localSecurityScoringHint: 'Highly specific, complete prompt-injection fingerprints are blocked locally at once. Other local rules still require two distinct nearby terms and an affirmative malicious verdict from secondary API review.',
      localSecurityObserveScore: 'Observe score',
      localSecurityObserveScoreHint: 'Record the risk evidence at this score without rejecting the request by itself.',
      localSecurityBlockScore: 'Mandatory review score',
      localSecurityBlockScoreHint: 'Ordinary local combinations trigger synchronous contextual API review at this score; complete high-specificity prompt-injection fingerprints bypass this score.',
      preBlockFailureMode: 'When audit is unavailable',
      preBlockFailureModeHint: 'Controls ordinary pre-block API failures. A failed ordinary combination review remains inconclusive; highly specific local prompt-injection fingerprints do not depend on the audit API.',
      preBlockFailureAllow: 'Continue forwarding (fail-open)',
      preBlockFailureBlock: 'Reject request (fail-closed)',
      localSecurityRules: 'Local security combination rules',
      localSecurityRulesPlaceholder: '[\n  {\'{\'}\n    "rule_name": "software_activation_crack",\n    "enabled": true,\n    "score": 90,\n    "actions": ["crack", "bypass"],\n    "targets": ["activation", "license"]\n  {\'}\'}\n]',
      localSecurityRulesHint: 'JSON array. Local rules nominate content for API review; they do not block directly. Mandatory review requires at least two distinct nearby terms, such as an action plus a target or two "all" terms. A lone exact term is observation-only. Optional score (1-100) overrides the rule default. Only models within the configured "Model scope" are checked.',
      localSecurityRulesInvalidJSON: 'Local security rules must be a valid JSON array.',
      localSecurityWhitelist: 'Security audit account whitelist',
      ...
      keywordModeKeywordAndApiDesc: 'Complete high-confidence prompt-injection fingerprints are blocked locally; other nearby two-keyword combinations trigger mandatory secondary API review, while remaining requests use normal moderation.',
      keywordModeKeywordOnly: 'Keyword only',
      keywordModeKeywordOnlyDesc: 'Complete high-specificity prompt-injection fingerprints are blocked locally; other cases call the secondary API only when two distinct nearby keywords form a combination.',
      keywordModeKeywordOnlyNotice: 'Keyword-only strategy: complete high-confidence prompt-injection fingerprints are blocked locally; other cases still require two distinct nearby keywords plus a high-confidence secondary API verdict.',
      keywordModeApiOnly: 'API only',
      keywordModeApiOnlyDesc: 'Use the upstream moderation API for ordinary content; custom keywords are ignored, while built-in high-confidence prompt-injection fingerprints still block locally.',
      keywordModeApiOnlyNotice: 'API-only strategy: custom keywords are ignored; built-in high-confidence prompt-injection fingerprints are still rejected locally.',
      overview: {
        status: 'Status',
        enabled: 'Enabled',
        disabled: 'Disabled',
      ...
      filters: {
        search: 'Search user/key/summary/rule/error',
        from: 'From',
        to: 'To',
        allGroups: 'All Groups',
      ...
      action: {
        block: 'Blocked',
        upstreamPolicyBlock: 'Upstream policy blocked',
        promptGuardBlock: 'Prompt Guard blocked',
        sessionPolicyBlock: 'Blocked policy session',
        keywordBlock: 'Local Rule Blocked',
