export default {
    ...
    riskControl: {
      ...
      localSecurityScoringHint: 'Local rules only identify review candidates. At least two distinct nearby terms are required for mandatory API review, and the request is rejected only when that second review clearly confirms malicious intent.',
      ...
      localSecurityBlockScoreHint: 'Trigger synchronous contextual API review at this score; the local score alone never rejects the request.',
      preBlockFailureMode: 'When audit is unavailable',
      preBlockFailureModeHint: 'Controls ordinary pre-block API failures. A failed local combination review is always inconclusive and cannot block by itself.',
      ...
      keywordModeKeywordAndApiDesc: 'Two distinct nearby keywords trigger mandatory secondary API review; other requests still use the normal moderation API. Blocking requires a high-confidence malicious verdict.',
      keywordModeKeywordOnly: 'Keyword only',
      keywordModeKeywordOnlyDesc: 'Call the secondary API only when two distinct nearby keywords form a combination; otherwise allow. Keywords never block by themselves.',
      keywordModeKeywordOnlyNotice: 'Keyword-only strategy: one keyword does not trigger review. Two distinct nearby keywords must form a combination, followed by a high-confidence malicious verdict from the secondary API, before blocking.',
