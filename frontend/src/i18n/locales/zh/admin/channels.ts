export default {
    ...
    riskControl: {
      ...
      refresh: '刷新',
      config: '内容审计配置',
      configHint: '调用已配置的 Moderations 或 Chat Completions 审核接口进行请求内容评分，命中阈值后按模式处理。',
      openSettings: '内容审计设置',
      ...
      enabled: '开启内容审计',
      enabledHint: '关闭后不调用审核 API 或异步队列；风控中心总开关开启时，高度特异的本地提示注入指纹仍会立即拦截。',
      mode: '全局模式',
      modePreBlock: '前置拦截',
      ...
      modeOff: '关闭',
      modeOffDesc: '不执行审核 API 或异步审计；风控中心开启时，高度特异的本地提示注入指纹仍会被本地拦截并记录。',
      auditAPIFormat: '审核接口类型',
      auditAPIFormatOpenAI: 'OpenAI Moderations',
      auditAPIFormatChat: 'Chat Completions（DeepSeek）',
      auditAPIFormatOpenAIDesc: '调用 /v1/moderations，适用于 OpenAI 及兼容审核服务。',
      auditAPIFormatChatDesc: '调用 /chat/completions 做上下文复审。本地组合词二审失败时不会单独拦截；普通前置审核失败按下方策略处理。',
      ...
      auditErrorReason: '审核异常原因',
      matchedKeyword: '命中关键词 / 规则',
      queueDelay: '排队 {ms} ms',
      allGroups: '全部分组',
      ...
      cleanupStats: '上次清理删除命中/异常 {hit} 条，未命中 {nonHit} 条',
      ...
      blockedKeywords: '复审关键词',
      blockedKeywordsPlaceholder: '每行输入一个关键词，例如：\n敏感词1\n敏感词2',
      blockedKeywordsDescription: '匹配忽略大小写；至少两个不同关键词邻近组合命中才会触发二次 API 复审，单个词不会直接拦截。',
      blockedKeywordsPreBlockHint: '关键词组合复审仅在「前置拦截」模式下生效。',
      blockedKeywordsModeWarning: '当前为「{mode}」模式，关键词组合复审不会生效；请切换到「前置拦截」模式后再保存关键词。',
      blockedKeywordCount: '已配置 {count} 个关键词',
      blockedKeywordsLimit: '最多保存 {max} 个关键词，单个长度不超过 200 个字符；重复项会自动去重。',
      localSecurityScoring: '本地安全风险评分',
      localSecurityScoringHint: '高度特异的完整提示注入指纹会在本地立即拦截；其他本地规则仍需至少两个不同词邻近组合后调用二次审核 API，且只有二审明确确认恶意才拒绝。',
      localSecurityObserveScore: '观察分数',
      localSecurityObserveScoreHint: '达到该分数时记录风险证据，但不单独拒绝请求。',
      localSecurityBlockScore: '强制复审分数',
      localSecurityBlockScoreHint: '普通本地组合达到该分数时同步调用上下文二审 API；高度特异的完整提示注入指纹不受该分数限制。',
      preBlockFailureMode: '审核不可用时',
      preBlockFailureModeHint: '控制普通前置审核 API 无 Key、超时或报错时的行为；普通本地组合词二审失败视为结论不明，高度特异的本地提示注入指纹不依赖审核 API。',
      preBlockFailureAllow: '继续转发（Fail-open）',
      preBlockFailureBlock: '拒绝请求（Fail-closed）',
      localSecurityRules: '本地安全组合规则',
      localSecurityRulesPlaceholder: '[\n  {\'{\'}\n    "rule_name": "software_activation_crack",\n    "enabled": true,\n    "score": 90,\n    "actions": ["破解", "绕过"],\n    "targets": ["激活码", "注册码"]\n  {\'}\'}\n]',
      localSecurityRulesHint: '填写 JSON 数组。本地规则只筛选 API 复审候选，不直接拦截。强制复审至少需要两个不同词邻近组合，例如 action + target 或两个 all 条件；单个 exact 仅作观察。可选 score（1-100）覆盖默认风险分。仅对“模型范围”内的模型执行。',
      localSecurityRulesInvalidJSON: '本地安全规则必须是有效的 JSON 数组。',
      localSecurityWhitelist: '安全审计账号白名单',
      ...
      keywordModeKeywordAndApiDesc: '完整的高置信提示注入指纹由本地立即拦截；其他邻近双关键词组合强制二次 API 复审，其余请求仍按常规审核。',
      keywordModeKeywordOnly: '仅关键词',
      keywordModeKeywordOnlyDesc: '高度特异的完整提示注入指纹本地立即拦截；其他情况仅在两个不同且邻近的关键词形成组合时调用二次 API。',
      keywordModeKeywordOnlyNotice: '当前为「仅关键词」策略：完整的高置信提示注入指纹会本地立即拦截；其他情况仍需两个不同且邻近的关键词形成组合，再由二次 API 高置信确认。',
      keywordModeApiOnly: '仅 API',
      keywordModeApiOnlyDesc: '普通内容只调用上游审计接口；自定义关键词列表不生效，但内置高置信提示注入指纹仍会本地立即拦截。',
      keywordModeApiOnlyNotice: '当前为「仅 API」策略：自定义关键词列表不会生效；内置高置信提示注入指纹仍由本地直接拦截。',
      overview: {
        status: '运行状态',
        enabled: '已启用',
        disabled: '未启用',
      ...
      action: {
        block: '拦截',
        upstreamPolicyBlock: '上游安全策略拦截',
        promptGuardBlock: '提示词安全审核拦截',
        sessionPolicyBlock: '安全策略会话拦截',
        keywordBlock: '本地强规则拦截',
        cyberPolicy: '网络安全策略',
        error: '异常',
      },
    },
