export default {
    ...
    riskControl: {
      ...
      localSecurityScoringHint: '本地规则只负责筛出复审候选。至少两个不同词在邻近范围内组合命中才会强制调用二次审核 API；只有二审明确确认恶意时才拒绝请求。',
      localSecurityObserveScore: '观察分数',
      localSecurityObserveScoreHint: '达到该分数时记录风险证据，但不单独拒绝请求。',
      localSecurityBlockScore: '强制复审分数',
      localSecurityBlockScoreHint: '达到该分数时同步调用上下文二审 API；本地分数本身不会直接拒绝请求。',
      preBlockFailureMode: '审核不可用时',
      preBlockFailureModeHint: '控制普通前置审核 API 无 Key、超时或报错时的行为；本地组合词二审失败一律视为结论不明，不能单独拦截。',
      ...
      keywordModeKeywordAndApiDesc: '两个不同关键词在邻近范围内组合命中时强制二次 API 复审；其他请求仍按常规调用审核 API。只有二审高置信确认恶意才拦截。',
      keywordModeKeywordOnly: '仅关键词',
      keywordModeKeywordOnlyDesc: '仅组合命中两个不同且邻近的关键词时调用二次 API 复审；未形成组合则放行。关键词本身不会直接拦截。',
      keywordModeKeywordOnlyNotice: '当前为「仅关键词」策略：单个关键词不会触发审核；只有两个不同且邻近的关键词形成组合时才调用二次 API，高置信确认恶意后才拦截。',
