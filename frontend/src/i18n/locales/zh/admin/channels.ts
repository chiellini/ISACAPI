export default {
    ...
    riskControl: {
      ...
      localSecurityRulesHint: '填写 JSON 数组。内置本地规则仍会保持启用；单条规则支持 exact 任一命中、all 全部命中，或 actions 任一词与 targets 任一词组合命中。规则会在计费、路由和上游调用前执行。',
      localSecurityRulesInvalidJSON: '本地安全规则必须是有效的 JSON 数组。',
      localSecurityWhitelist: '本地过滤账号白名单',
      localSecurityWhitelistPlaceholder: '每行填写一个用户 ID，例如：\n10001\n10002',
      localSecurityWhitelistHint: '白名单用户仅绕过本地政治、逆向和 cyber 关键词层；上游内容审计、计费、路由及其他账号风控仍然生效。',
      localSecurityWhitelistInvalid: '白名单中的每一项必须是正整数用户 ID。',
