export default {
    ...
    riskControl: {
      ...
      localSecurityRulesHint: '填写 JSON 数组。内置本地规则仅拦截软件破解与逆向工程意图，且要求动作词与目标词邻近出现。单条规则支持 exact 任一命中、all 全部命中，或 actions 任一词与 targets 任一词邻近组合命中。仅对“模型范围”配置内的模型执行。规则会在计费、路由和上游调用前执行。',
