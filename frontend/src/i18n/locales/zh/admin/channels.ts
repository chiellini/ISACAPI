export default {
    ...
    riskControl: {
      ...
      auditAPIFormatChatDesc: '调用 /chat/completions 进行灰区复审；超时、无 Key、网络错误或结果无法解析时直接阻断。',
      ...
      localSecurityScoringHint: '不同本地规则的精确命中或组合命中会累积风险分。达到拦截分数时立即拒绝；达到观察分数但未拦截时会记录结构化日志并继续由后续审核链路判断。',
      localSecurityObserveScore: '观察分数',
      localSecurityObserveScoreHint: '达到该分数时记录风险证据，但不单独拒绝请求。',
      localSecurityBlockScore: '拦截分数',
      localSecurityBlockScoreHint: '达到该分数时在计费、路由和上游调用前拒绝请求。',
      preBlockFailureMode: '审核不可用时',
      preBlockFailureModeHint: '仅影响前置审核 API 无 Key、超时或报错的情况；本地硬规则仍始终优先执行。',
      ...
      localSecurityRulesHint: '填写 JSON 数组。内置本地规则仅拦截软件破解与逆向工程意图，且要求动作词与目标词邻近出现。单条规则支持 exact 任一命中、all 全部命中，或 actions 任一词与 targets 任一词邻近组合命中。可选 score（1-100）覆盖该规则的默认风险分；远距离组合会降为观察分。仅对“模型范围”配置内的模型执行。规则会在计费、路由和上游调用前执行。',
